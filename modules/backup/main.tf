# AWS Backup Module with Cross-Region Support
# Provides automated backup for RDS and other AWS resources
...
# AWS Backup Vault in primary region (us-east-1)
...
# AWS Backup Vault in cross-region (sa-east-1)
...
# Attach AWS managed policy for restoring from backup
...
# Backup Plan for daily backups with cross-region copy
resource "aws_backup_plan" "daily" {
  count = var.environment == "prod" ? 1 : 0
  
  name = "${var.environment}-daily-backup-plan"
  
  # Primary backup rule (daily)
  rule {
  ...
  rule {
    ...
    # Cross-region copy for weekly backups
    copy_action {
      destination_vault_arn = aws_backup_vault.cross_region[0].arn
...
# CloudWatch Metric Filter for failed backups
...
# Report Plan for backup compliance
